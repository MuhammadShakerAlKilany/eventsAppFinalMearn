...
import { login, register ,banUser ,varifyUser ,allUser,allUserBan, edite, editeUserWithAdmin, deleteUser, addUser, getUserProPicPath, changPlane, successPlane, closPlane, userData} from "../controller/user.controller";
...
router.route("/").all(guard).patch(upload.single("proPic"),joiValidatorBody(userUpdateSchema),edite).get(userData)
